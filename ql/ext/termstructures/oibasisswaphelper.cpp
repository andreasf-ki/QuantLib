...
void OIBSHelper::initializeDates() {
    ...
    ext::shared_ptr<IborIndex> clonedIborIndex = iborIndex_->clone(termStructureHandle_);
    // avoid notifications
    ...
    swap_ = ext::shared_ptr<OvernightIndexedBasisSwap>(new OvernightIndexedBasisSwap(OvernightIndexedBasisSwap::Payer,
                                                                                       10000.0, // arbitrary
                                                                                       oisSchedule, overnightIndex_,
                                                                                       iborSchedule, clonedIborIndex));
    ext::shared_ptr<PricingEngine> engine(new DiscountingSwapEngine(overnightIndex_->forwardingTermStructure()));
