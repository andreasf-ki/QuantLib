...
void OICCBSHelper::initializeDates() {
    ...
    ext::shared_ptr<Quote> fx(new SimpleQuote(1.0));
    swap_ = ext::shared_ptr<OvernightIndexedCrossCcyBasisSwap>(
    ...
    if (fixedDiscountOnPayLeg_) {
        ext::shared_ptr<PricingEngine> engine(new OvernightIndexedCrossCcyBasisSwapEngine(
