...
AverageONLeg::AverageONLeg(const Schedule& schedule, const ext::shared_ptr<OvernightIndex>& i)
...
AverageONLeg::operator Leg() const {
    ...
    for (Size i = 0; i < numPeriods; ++i) {
        ...
        ext::shared_ptr<AverageONIndexedCoupon> cashflow(new AverageONIndexedCoupon(
