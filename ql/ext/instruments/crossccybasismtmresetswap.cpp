...
CrossCcyBasisMtMResetSwap::CrossCcyBasisMtMResetSwap(
    ...
    const ext::shared_ptr<IborIndex>& foreignIndex, Spread foreignSpread, const Currency& domesticCurrency,
    const Schedule& domesticSchedule, const ext::shared_ptr<IborIndex>& domesticIndex, Spread domesticSpread,
    const ext::shared_ptr<FxIndex>& fxIdx, bool invertFxIdx, bool receiveDomestic)
...
void CrossCcyBasisMtMResetSwap::initialize() {
    ...
    ext::shared_ptr<CashFlow> initialPayCF(new SimpleCashFlow(-foreignNominal_, initialPayDate));
    legs_[0].insert(legs_[0].begin(), initialPayCF);
    // Pay leg notional exchange at end.
    Date finalPayDate = foreignSchedule_.dates().back();
    ext::shared_ptr<CashFlow> finalPayCF(new SimpleCashFlow(foreignNominal_, finalPayDate));
    ...
    for (Size j = 0; j < legs_[1].size(); ++j) {
        ext::shared_ptr<FloatingRateCoupon> coupon = ext::dynamic_pointer_cast<FloatingRateCoupon>(legs_[1][j]);
        Date fixingDate = fxIndex_->fixingCalendar().advance(coupon->accrualStartDate(),
                                                             -static_cast<Integer>(fxIndex_->fixingDays()), Days);
        ext::shared_ptr<FloatingRateFXLinkedNotionalCoupon> fxLinkedCoupon(
    ...
    for (Size j = 0; j < legs_[1].size(); j++) {
        ext::shared_ptr<Coupon> c = ext::dynamic_pointer_cast<Coupon>(legs_[1][j]);
        ...
        Date fixingDate = fxIndex_->fixingCalendar().advance(c->accrualStartDate(),
                                                             -static_cast<Integer>(fxIndex_->fixingDays()), Days);
        legs_[2].push_back(ext::shared_ptr<CashFlow>(
