...
OvernightIndexedCrossCcyBasisSwap::OvernightIndexedCrossCcyBasisSwap(
    ...
    const ext::shared_ptr<OvernightIndex>& payIndex, Real paySpread, Real recNominal, Currency recCurrency,
    const Schedule& recSchedule, const ext::shared_ptr<OvernightIndex>& recIndex, Real recSpread)
...
void OvernightIndexedCrossCcyBasisSwap::initialize() {
    ...
    legs_[0].insert(legs_[0].begin(),
                    ext::shared_ptr<CashFlow>(new SimpleCashFlow(-payNominal_, paySchedule_.dates().front())));
    legs_[0].push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(payNominal_, paySchedule_.dates().back())));

    legs_[1] = OvernightLeg(recSchedule_, recIndex_).withNotionals(recNominal_).withSpreads(recSpread_);
    legs_[1].insert(legs_[1].begin(),
                    ext::shared_ptr<CashFlow>(new SimpleCashFlow(-recNominal_, recSchedule_.dates().front())));
    legs_[1].push_back(ext::shared_ptr<CashFlow>(new SimpleCashFlow(recNominal_, recSchedule_.dates().back())));
